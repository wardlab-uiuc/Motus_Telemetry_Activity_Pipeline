...
#   further with maps, timelines, receiver metadata, and biological context.
###############################################################################
...
# Activity_Timing_Functions.R should include:
#   - info_fast()
#   - classify_activity()
#
# Diagnostic_Plots_Functions.R should include:
#   - make_plot_id()
...
# Parent folder containing all individual MotusTagID × mfgID folders created by Step 1.
  ...
  Tower_metadata %>%
    ...
    bind_rows(
      Tower_metadata %>%
        ...
        filter(!is.na(System2), !is.na(System1End)) %>%
        transmute(
          recvDeployName,
...
# ==============================================================================
# 7) Loop through all MotusTagID × mfgID folders
# ==============================================================================

for (data_dir in dataset_folders) {
  
  message("\n============================================================")
  message("📁 Processing folder: ", basename(data_dir))
  message("============================================================")
  
  # ---------------------------------------------------------------------------
  # Parse input identifiers and load data
  # ---------------------------------------------------------------------------
  ...
  if (!file.exists(file_path)) {
    message("  ⚠️ Skipping folder because detection file was not found: ", file_path)
    next
  }
  ...
  if (nrow(bird_deployments) == 0) {
    message("  ⚠️ Skipping: no matching bird metadata found.")
    next
  }
  ...
  # ============================================================================
  # Process each biological deployment
  # ============================================================================
    ...
    if (run_stationary_tag_screen) {
      ...
      if (!is.null(stationary_screen)) {
        if (isTRUE(stationary_screen$summary$flagged_possible_stationary_tag)) {
          message("  ⚠️ Stationary-tag screen FLAGGED this deployment. Manual review required.")
        } else {
          message("  ✅ Stationary-tag screen did not flag this deployment.")
        }
      } else {
        message("  ⚪ Stationary-tag screen skipped: not enough valid late-track daytime detections.")
      }
    }
...
message("\n✅ ALL DATASET FOLDERS AND DEPLOYMENTS COMPLETE — TABLES + PLOTS GENERATED")
message("\n🎉 STEP 3 COMPLETE :)")